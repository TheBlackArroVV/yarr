...
) -> Option<Result<Vec<Entry>, ParserError>> {
    ...
    loop {
        match reader.read_event(&mut buf) {
            Ok(Event::Start(ref e)) => match e.name() {
                ...
                b"updated" => match reader.read_event(&mut buf) {
                    Ok(Event::Text(e)) => {
                        current_entry.updated = e.unescape_and_decode(&reader).unwrap();
                    }
                    _ => (),
                },
                b"author" => match reader.read_event(&mut buf) {
                    Ok(Event::Start(ref e)) => match e.name() {
                        b"name" => match reader.read_event(&mut buf) {
                            Ok(Event::Text(e)) => {
            ...
            Ok(Event::Empty(ref e)) => {
                if e.name() == b"link" {
                    match e
                        .attributes()
                        .with_checks(false)
                        .find(|a| a.as_ref().unwrap().key == b"href")
                    {
                        Some(attr) => {
                            current_entry.link =
                                attr.unwrap().unescape_and_decode_value(&reader).unwrap();
                        }
                        _ => (),
                    }
                }
